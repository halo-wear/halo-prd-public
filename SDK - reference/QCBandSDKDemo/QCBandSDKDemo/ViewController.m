...
//    }];
    ...
    [QCSDKCmdCreator getFulldaySleepDetailDataByDay:1 sleepDatas:^(NSArray<QCSleepModel *> * _Nullable sleeps, NSArray<QCSleepModel *> * _Nullable naps) {
...
- (void)hrMeasuringAction {
    ...
    [QCSDKCmdCreator setTime:[NSDate date] success:^(NSDictionary * _Nonnull info) {
        ...
        if([[info objectForKey:QCBandFeatureAppManual] boolValue]) {
            ...
            QCMeasuringType operateType = QCMeasuringTypeHRV;
            [[QCSDKManager shareInstance] startToMeasuringWithOperateType:operateType measuringHandle:^(id  _Nullable result) {
                ...
                NSLog(@"HRV measurement results：%@",result);
                if (isSuccess) {
                    NSLog(@"HRV measurement：%zd",[(NSNumber*)result integerValue]);
